...
use self::paths::{find_it, CargoPathExt};
...
impl Project {
    ...
    /// `kind` should be one of: "lib", "rlib", "staticlib", "dylib",
    /// "proc-macro" ex: `/path/to/cargo/target/cit/t0/foo/target/debug/
    /// examples/libex.rlib`
    ...
    /// ex: `/path/to/cargo/target/cit/t0/foo/target/i686-apple-darwin/debug/
    /// foo`
    ...
    /// Creates a `ProcessBuilder` to run xargo.
    /// Arguments can be separated by spaces.
    /// Example:
    ///     p.cargo("build --bin foo").run();
    pub fn xargo(&self, cmd: &str) -> Execs {
        let mut execs = self.process(&xargo_exe());
        if let Some(ref mut p) = execs.process_builder {
            split_and_add_args(p, cmd);
        }
        execs
...
pub fn cargo_dir() -> PathBuf {
    ...
    if let Some(path) = find_it("cargo") {
        if let Some(path) = path.parent() {
            ret = path.to_path_buf();
        }
    }
    if !ret.exists() {
        panic!("CARGO wasn't set in PATH. Cannot continue running test")
    }
    ret
}

pub fn xargo_dir() -> PathBuf {
    let mut ret = PathBuf::new();
    if let Some(path) = find_it("xargo") {
        if let Some(path) = path.parent() {
            ret = path.to_path_buf();
        }
    }
    if !ret.exists() {
        panic!("XARGO wasn't set in PATH. Cannot continue running test")
    }
    ret
}
...
pub fn xargo_exe() -> PathBuf {
    xargo_dir().join(format!("xargo{}", env::consts::EXE_SUFFIX))
}
...
///   platforms).
/// - There is a wide range of macros (such as `[COMPILING]` or `[WARNING]`) to
///   match cargo's "status" output and allows you to ignore the alignment. See
///   `substitute_macros` for a complete list of macros.
...
fn _process(t: &OsStr) -> cargo::util::ProcessBuilder {
    ...
    p.cwd(&paths::root())
        ...
        .env("__CARGO_SCQ_ROOT", paths::root())
        // Force Cargo to think it's on the stable channel for all tests, this
        // should hopefully not surprise us as we add cargo features over time and
        // cargo rides the trains.
        .env("__CARGO_SCQ_CHANNEL_OVERRIDE_DO_NOT_USE_THIS", "stable")
...
impl ChannelChanger for cargo::util::ProcessBuilder {
    fn masquerade_as_nightly_cargo(&mut self) -> &mut Self {
        self.env("__CARGO_SCQ_CHANNEL_OVERRIDE_DO_NOT_USE_THIS", "nightly")
    }
}
...
/// This provides a way for those setups to increase the cut off for all the
/// time based test.
