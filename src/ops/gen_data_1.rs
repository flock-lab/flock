...
async fn handler(event: KinesisEvent, _: Context) -> Result<Value, Error> {
    ...
    let projection: Vec<String> = vec!["attr_1", "attr_2", "attr_3", "attr_4"]
        .into_iter()
        .map(|s| s.to_owned())
        .collect();
...
mod tests {
    ...
    async fn handler_handles() {
        ...
        let data = handler(event, Context::default()).await.ok().unwrap();

        // assert!(handler(event, Context::default()).await.is_ok());
        println!("{:?}", data);
        // serde_json::to_writer(&File::create("gen_data1_output.json").ok().unwrap(), &data);
    }
}
