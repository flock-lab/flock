...
/// A reference-counted reference to a `SCQSchema`.
pub type SCQSchemaRef = Arc<SCQSchema>;

/// SCQSchema wraps an Arrow schema and adds relation names
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SCQSchema {
    /// Fields
    fields: Vec<SCQField>,
}

impl SCQSchema {
    /// Creates an empty `SCQSchema`
    pub fn empty() -> Self {
        Self { fields: vec![] }
    }

    /// Create a new `SCQSchema`
    pub fn new(fields: Vec<SCQField>) -> Result<Self> {
        ...
        // note that we need to sort the contents of the HashSet first so that errors
        // are deterministic
    ...
    /// Create a `SCQSchema` from an Arrow schema
    ...
    /// Combine two schemas
    pub fn join(&self, schema: &SCQSchema) -> Result<Self> {
    ...
    pub fn fields(&self) -> &Vec<SCQField> {
        &self.fields
    }

    /// Returns an immutable reference of a specific `Field` instance selected
    /// using an offset within the internal `fields` vector
    pub fn field(&self, i: usize) -> &SCQField {
        &self.fields[i]
    }
    ...
    pub fn field_with_name(&self, relation_name: Option<&str>, name: &str) -> Result<SCQField> {
    ...
    /// Find the field with the given name
    pub fn field_with_unqualified_name(&self, name: &str) -> Result<SCQField> {
        let matches: Vec<&SCQField> = self
            .fields
            .iter()
    ...
    pub fn field_with_qualified_name(&self, relation_name: &str, name: &str) -> Result<SCQField> {
        let matches: Vec<&SCQField> = self
            .fields
            .iter()
            .filter(|field| {
...
impl Into<Schema> for SCQSchema {
    fn into(self) -> Schema {
        Schema::new(
            self.fields
                .iter()
...
impl Into<SchemaRef> for SCQSchema {
    fn into(self) -> SchemaRef {
        SchemaRef::new(self.into())
    }
}

impl Display for SCQSchema {
...
/// SCQField wraps an Arrow field and adds an optional qualifier
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SCQField {
...
impl SCQField {
    ...
    /// Returns an immutable reference to the `SCQField`'s unqualified name
    pub fn name(&self) -> &String {
        &self.field.name()
    }

    /// Returns an immutable reference to the `SCQField`'s data-type
    pub fn data_type(&self) -> &DataType {
        &self.field.data_type()
    }

    /// Indicates whether this `SCQField` supports null values
    pub fn is_nullable(&self) -> bool {
        self.field.is_nullable()
    }

    /// Returns a reference to the `SCQField`'s qualified name
...
mod tests {
    ...
    fn from_qualified_field() {
        let field = Field::new("c0", DataType::Boolean, true);
        let field = SCQField::from_qualified("t1", field);
        assert_eq!("c0", field.name());
    ...
    fn from_unqualified_schema() -> Result<()> {
        let schema = SCQSchema::from(&test_schema_1())?;
    ...
    fn from_qualified_schema() -> Result<()> {
        let schema = SCQSchema::from_qualified("t1", &test_schema_1())?;
    ...
    fn join_qualified() -> Result<()> {
        let left = SCQSchema::from_qualified("t1", &test_schema_1())?;
        let right = SCQSchema::from_qualified("t2", &test_schema_1())?;
    ...
    fn join_qualified_duplicate() -> Result<()> {
        let left = SCQSchema::from_qualified("t1", &test_schema_1())?;
        let right = SCQSchema::from_qualified("t1", &test_schema_1())?;
        let join = left.join(&right);
        assert!(join.is_err());
        assert_eq!(
    ...
    fn join_unqualified_duplicate() -> Result<()> {
        let left = SCQSchema::from(&test_schema_1())?;
        let right = SCQSchema::from(&test_schema_1())?;
        let join = left.join(&right);
        assert!(join.is_err());
        assert_eq!(
            "Error during planning: Schema contains duplicate \
    ...
    fn join_mixed() -> Result<()> {
        let left = SCQSchema::from_qualified("t1", &test_schema_1())?;
        let right = SCQSchema::from(&test_schema_2())?;
        let join = left.join(&right)?;
